...
- **macOS / Windows 自定义标题栏**：管理面板在 macOS 和 Windows 上使用前端自绘的窗口标题栏，主色带从左到右以 5% 到 70% 的不透明度叠加深 Gitea 绿，并保留毛笔笔触纹理；dev 构建切换为同样规则的低亮度鲸眼红色系，Linux 暂保留系统标题栏。两端的窗口按钮按各自平台惯例分别绘制：macOS 是左上角红黄绿交通灯；Windows 是右侧的最小化 / 关闭按钮（46×32 命中区、10px 细线字形、hover 覆浅色底、关闭 hover 变系统红，与 Windows 11 标准标题栏一致；窗口不可缩放故不提供最大化按钮）。
