...
- **Windows 常驻通知区域**：Windows 上管理面板是常驻后台的——标题栏的关闭与最小化按钮都只是把窗口收进通知区域，并从任务栏移除它的按钮（`ITaskbarList::DeleteTab`，任务栏与 Alt+Tab 都不再留一个点了没反应的窗口），内核、工作台、官方对话与更新检查继续运行；托盘图标（右键菜单「显示主界面 / 退出 dsh-xlink」，左键单击直接叫回窗口）是重新打开与退出的唯一入口，真正退出走托盘菜单并复用「确认退出」流程（内核运行时会先询问，确认后依次停止内核、销毁窗口、退出进程）。窗口收起时会提示一次「已收起到通知区域」，避免看起来像崩溃。
