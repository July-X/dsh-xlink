...
![v0.3.0 概览：左侧管理面板「当前内核」卡显示近 7 天用量，右侧模型用量窗口（90 天聚合 + 吸附跟随）](docs/images/v0.3.0-overview.jpg)

**v0.3.0 重点**：把模型用量做成 90 天增量聚合的独立窗口（默认吸附主窗右侧、主窗拖动时 60fps 跟随），日志查看器共用同一套吸附跟随；日志侧栏改成分组签 + 共享侧栏的可拖分栏。

## v0.3.0 变更摘要

- **模型用量统计窗口**：本地会话文件 → 增量账目 → 摘要 + 热力图 + 堆叠趋势 + 环形图列表
  - 6 档范围（今日 / 7 天 / 15 天 / 30 天 / 60 天 / 90 天），摘要卡含今日用量、日均、请求次数、活跃天数与最常用模型
  - 环形图中心显示当前范围的 token 合计，列表给出每模型 token 数与占比
  - 窗口可独立缩放；默认吸附主窗右侧、高度与本体一致；主窗拖动时以 60fps 合帧跟随，松手后停追
- **日志查看器**：接入与用量窗口同源的吸附 + 移动跟随；日志分隔条拖拽改为 pointer + rAF（关 transition）以消除卡顿
- **日志侧栏分类**：分组签 + 共享侧栏 + 可拖分栏，筛选粒度更细
- **插件页**：卡片头去掉与 tab 重复的「已安装」标题
...
- **日志查看器吸附跟随**：v0.3.0 起日志查看器接入与用量窗口同源的吸附 + 移动跟随逻辑——新建或调整大小的日志窗口默认吸附主窗右侧、高度与本体一致，主窗拖动时 60fps 合帧跟随，松手后停止；窗口始终独立可缩放，分隔条拖拽改为 pointer + rAF（关 transition）消除卡顿。
