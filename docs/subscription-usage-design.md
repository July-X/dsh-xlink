...
- 周层：`current_weekly_status == 1` 时按百分比激活；`status == 3` 表示该套餐
  **无周限额**——产出显式的 `unlimited` 周层，前端渲染「7d ♾️ 无限周额度」
  （不渲染进度条，避免被当成「有周额度但恰好满格」）。
...
- **重置倒计时**：紧凑格式 `1d15h` / `3h47m`，前置倒计时 icon（Timer）；
  完整中文描述（`1 天 15 小时后重置`）放 hover title。格式化函数在
  `ui/src/labels.js`（时间相关展示的既有去处），供卡片与窗口共用。
- **tier 短名**：进度条行名用 `5h` / `7d`（真机反馈「5 小时窗口 / 本周窗口」
  过长）；无限周额度以「♾️ 无限周额度」文本替代进度条。
