...
2. 逐文件还原：
   - 备份存在 → 用备份内容原子覆盖目标，删除备份（还原成功）。**唯一的例外是
     "伪造备份"**：rc.18 之前的外壳在「目标已经是补丁后内容」时也照常备份一次，
     于是备份里存的是补丁内容本身，按它还原等于什么都没做却报「已撤销」。判据是
     「记录没有 `originalSha256`（旧记录）+ 备份哈希等于记录的应用后哈希」两条同时
     成立（避免误伤"补丁恰好是空操作"的正常记录），命中则中止并提示重新安装该内核
     版本，或选择「清除记录」保留现状——这类旧记录无法事后修复，只能如实拦下。
