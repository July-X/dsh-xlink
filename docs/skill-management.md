...
> **面板现状**：启停的**后端能力已完整实现**（`skill_set_enabled` 命令 + `enabled: false` 语义 + 启动对账），但 v1 管理面板只提供安装 / 卸载 / 更新 / 重新同步，**尚未接线启停按钮**（与 README「v1 面板只出手动安装行」一致）。下面描述的是命令层语义；面板接线之前，启停只能通过直接调用该命令完成。
