...
mod tests {
    ...
    /// 端到端：`LogSpec::with_instance` + `path_for` 链式产出实例感知文件名，
    /// 与 kernel.rs / kernel_adapter.rs / guard.rs 等 caller 的实际写法对齐
    /// （kernel_log_spec(family, id).path_for(...)）。这是 dev plan §4
    /// release threshold 修复合入后的回归保险。
    #[test]
